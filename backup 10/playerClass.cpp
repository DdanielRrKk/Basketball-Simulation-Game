...
void Player::setPlayerOverall() { //mp_overall = ((7 * mp_insideScoring) + (8 * mp_outsideScoring) + mp_freeThrow + (6 * mp_passing) + (5 * mp_rebounding) + (2 * mp_stealing) + (3 * mp_blocking) + (4 * mp_defending)) / 30; 
	...
	if (mp_position == "PG" || mp_position == "PG/SG") {
		mp_overall = ((8 * mp_insideScoring) + (24 * mp_outsideScoring) + (10 * mp_freeThrow) + (15 * mp_passing) + (7 * mp_rebounding) + (12 * mp_stealing) + (4 * mp_blocking) + (10 * mp_defending)) / 76;
	}

	if (mp_position == "SG" || mp_position == "SG/PG" || mp_position == "SG/SF") {
		mp_overall = ((10 * mp_insideScoring) + (21 * mp_outsideScoring) + (11 * mp_freeThrow) + (11 * mp_passing) + (10 * mp_rebounding) + (10 * mp_stealing) + (7 * mp_blocking) + (10 * mp_defending)) / 76;
	}

	if (mp_position == "SF" || mp_position == "SF/SG" || mp_position == "SF/PF") {
		mp_overall = ((15 * mp_insideScoring) + (15 * mp_outsideScoring) + (9 * mp_freeThrow) + (12 * mp_passing) + (11 * mp_rebounding) + (8 * mp_stealing) + (8 * mp_blocking) + (12 * mp_defending)) / 76;
	}
	
	if (mp_position == "PF" || mp_position == "PF/SF" || mp_position == "PF/C") {
		mp_overall = ((21 * mp_insideScoring) + (12 * mp_outsideScoring) + (7 * mp_freeThrow) + (9 * mp_passing) + (12 * mp_rebounding) + (7 * mp_stealing) + (10 * mp_blocking) + (12 * mp_defending)) / 76;
	}

	if (mp_position == "C" || mp_position == "C/PF") {
		mp_overall = ((24 * mp_insideScoring) + (8 * mp_outsideScoring) + (4 * mp_freeThrow) + (7 * mp_passing) + (15 * mp_rebounding) + (4 * mp_stealing) + (12 * mp_blocking) + (16 * mp_defending)) / 76;
	}
}
